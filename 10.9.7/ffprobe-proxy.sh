...
execute_macos_ffmpeg() {
    log "Forwarding FFmpeg Command to macOS: $cmd_with_mapped_paths"
    
    sshpass -p "$MACOS_PASSWORD" ssh -o StrictHostKeyChecking=no "$MACOS_USER@macos_host" "$MACOS_FFMPEG $@" 1>&1 2>&2
    exit $?  
}
